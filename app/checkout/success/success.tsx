  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        // Cast the document data to include an optional 'slaves' array so TypeScript recognizes it
        const order = { id: orderSnap.id, ...(orderSnap.data() as { slaves?: string[]; [key: string]: any }) };
        setOrderData(order);
  ...
  if (!orderId) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-red-500">No order ID found in the URL.</p>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-sm text-foreground/60">
            Order number:{" "}
            <span className="font-mono font-semibold text-accent">{orderData.id}</span>
          </p>
        </div>
